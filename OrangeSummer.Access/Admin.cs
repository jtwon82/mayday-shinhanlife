...
namespace OrangeSummer.Access
{
    ...
    public class Admin
    {
        ...
        public Model.Admin Login202302(string usr, string pwd)
        {
            //List<Model.Admin> lists = null;
            Model.Admin r = null;

            StringBuilder query = new StringBuilder();
            query.Append($"select A.ID,dbo.UFNC_DECRYPT(A.USR)USR, dbo.UFNC_DECRYPT(A.PWD)PWD, dbo.UFNC_DECRYPT(A.PHONE)PHONE, A.NAME, A.RESET ,A.DEL_YN ,A.REGIST_DATE from ADMIN A WHERE dbo.UFNC_DECRYPT([USR]) = '{usr}'  ");
            ...
            using (DataTable dt = DBHelper.ExecuteDataTableInQuery(_connection, query.ToString()))
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        Model.Admin t = new Model.Admin().getAdmin(dr);
                        if(t.Pwd== pwd)
                        {
                            r = t;
                        }
