...
namespace OrangeSummer.Access
{
    ...
    public class Word
    {
        ...
        public List<Model.Word> List2(int page, int size)
        {
            ...
            parameters.Add(new SqlParameter("@PAGE", page));
            parameters.Add(new SqlParameter("@SIZE", size));
            using (DataTable dt = DBHelper.ExecuteDataTable(_connection, "ADM_WORD_LIST2", parameters))
            {
                if (dt.Rows.Count > 0)
                {
                    lists = new List<Model.Word>();
        ...
        {
            List<Model.Word> lists = null;
            using (DataTable dt = DBHelper.ExecuteDataTable(_connection, "ADM_WORD_EXCEL2"))
            {
                if (dt.Rows.Count > 0)
                {
                    lists = new List<Model.Word>();
        ...
        {
            List<Model.Word> lists = null;
            using (DataTable dt = DBHelper.ExecuteDataTable(_connection, "ADM_WORD_RANKING2"))
            {
                if (dt.Rows.Count > 0)
                {
                    lists = new List<Model.Word>();
        ...
        public string UserVote2(string member, string vote)
        {
            ...
            parameters.Add(new SqlParameter("@FK_MEMBER", member));
            parameters.Add(new SqlParameter("@VOTE", vote));
            using (DataTable dt = DBHelper.ExecuteDataTable(_connection, "USP_WORD_VOTE2", parameters))
            {
                if (dt.Rows.Count == 1)
                {
                    DataRow dr = dt.Rows[0];
